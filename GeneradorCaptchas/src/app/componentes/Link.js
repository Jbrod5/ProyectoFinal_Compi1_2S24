...
export class Link extends Componente{
        ...
        switch(parametro){
            case 'HREF':
                if(this.href == undefined){
                    ...
                    var mensaje = "Error Semantico: se intento establecer el parametro href pero ya estaba definido.";
                    super.lanzarExcepcionSemantica(mensaje)
                }
                break;
            default: 
                var mensaje = "Error Semantico: se intentó establecer el parametro " + parametro + " en una instancia Link pero no es un parametro valido para el componente.";
                super.lanzarExcepcionSemantica(mensaje);
        }
    }
     ...
     */
    obtenerCodigo(){
        var codigo = "<a href=\"" + this.href + "\" style=\" display: inline-block; padding: 10px 20px; background-color: #4CAF50; color: white; text-decoration: none; font-family: Arial, sans-serif; font-size: 16px; border-radius: 5px; transition: background-color 0.3s ease;\">" + this.valor + "</a>";
        return codigo;
    }
}